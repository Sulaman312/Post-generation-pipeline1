...
def save_run_manifest(
    ...
    *,
    ...
    payload = {
        "client_id": client_id,
        "run_id": run_id,
        "pipeline_id": (pipeline_id or prev.get("pipeline_id") or "social_media"),
        "topic": topic,
        "statuses": statuses,
...
def _infer_statuses_from_run_dir(run_dir: Path) -> dict[str, str]:
    statuses = {name: "pending" for name in _STEP_ORDER}
...
def _infer_topic_from_run_dir(run_dir: Path) -> str:
    topic_file = run_dir / "client_profile_topic.md"
    if topic_file.is_file():
        try:
            text = topic_file.read_text(encoding="utf-8").strip()
            if text:
                return text.split("\n", 1)[0][:500]
        except OSError:
            pass
...
def _repair_manifest_topic_if_needed(data: dict, run_dir: Path) -> dict:
    ...
    if isinstance(topic, str) and topic.strip() and topic.strip().lower() != "untitled":
        return data
    inferred = _infer_topic_from_run_dir(run_dir)
    if inferred and inferred != "untitled":
        data["topic"] = inferred
    return data
